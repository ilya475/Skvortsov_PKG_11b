...
import { createChannels, setMatrices, setModelValues, setPreview, setRangeGradients, setStatus, updatePicker } from './view.js';
...
function buildRangeGradients(model) {
  const configs = {
    rgb: [
      { min: 0, max: 255 }, { min: 0, max: 255 }, { min: 0, max: 255 }
    ],
    xyz: [
      { min: -20, max: 150 }, { min: -20, max: 150 }, { min: -20, max: 150 }
    ],
    hsv: [
      { min: 0, max: 360 }, { min: 0, max: 100 }, { min: 0, max: 100 }
    ]
  };

  const sourceValues = state[model].slice();
  return configs[model].map((range, channelIndex) => {
    const colors = [];
    const steps = 13;
    for (let i = 0; i < steps; i++) {
      const value = range.min + (range.max - range.min) * (i / (steps - 1));
      const sample = sourceValues.slice();
      sample[channelIndex] = value;
      let result;
      if (model === 'rgb') result = calculateFromRgb(sample, state.illuminant, state.gamutStrategy);
      if (model === 'xyz') result = calculateFromXyz(sample, state.illuminant, state.gamutStrategy);
      if (model === 'hsv') result = calculateFromHsv(sample, state.illuminant, state.gamutStrategy);
      colors.push(hexFromRgb(result.rgb));
    }
    return colors;
  });
}
